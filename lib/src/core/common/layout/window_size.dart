...
extension type const WindowSize(Size _size) implements Size {
  ...
  bool get isExpandedOrLarger => maybeMap(orElse: () => true, compact: () => false, medium: () => false);
  ...
  bool get isLargeOrLarger => maybeMap(orElse: () => false, large: () => true, extraLarge: () => true);
...
class const _InheritedWindowSize({
  /// The [WindowSize] provided by this scope.
  required final WindowSize windowSize,
