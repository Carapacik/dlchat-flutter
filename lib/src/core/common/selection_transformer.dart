...
class SelectionTransformer extends StatefulWidget {
  const new({required this.transform, required this.child, super.key});

  /// A [SelectionTransformer] that adds a separator string between each child selections.
  new separated({required this.child, super.key, String separator = '\n'})
  ...
  /// Body   Not Found
  ///
  new tabular({required int columns, required this.child, super.key}) : transform = _tabularSelectionTransform(columns);

  static SelectionTransform _separatedSelectionTransform(String separator) =>
      (selections) => selections.join(separator);

  static SelectionTransform _tabularSelectionTransform(int columns) => (selections) {
    final List<int> maxColumnWidths = List.filled(columns - 1, 0);
...
class SeparatedSelectionContainerDelegate(final SelectionTransform transform)
    extends MultiSelectableSelectionContainerDelegate {
  @override
  SelectedContent? getSelectedContent() {
    final selections = <SelectedContent>[];
    for (final Selectable selectable in selectables) {
      final Iterable<SelectedContent> allSelectedContent = getAllSelectables(selectable)
    ...
    return SelectedContent(plainText: transform(selections.map((s) => s.plainText)));
  }
  ...
  List<Selectable> getAllSelectables(Selectable selectable) {
    ...
    if (selectable is State<SelectionContainer>) {
      final SelectionContainer s = (selectable as State<SelectionContainer>).widget;
      if (s.delegate is MultiSelectableSelectionContainerDelegate) {
        final d = s.delegate! as MultiSelectableSelectionContainerDelegate;
  ...
  final _hasReceivedStartEvent = <Selectable>{};
  final _hasReceivedEndEvent = <Selectable>{};
  ...
  void ensureChildUpdated(Selectable selectable) {
    if (_lastEndEdgeUpdateGlobalPosition != null && _hasReceivedEndEvent.add(selectable)) {
      final synthesizedEvent = SelectionEdgeUpdateEvent.forEnd(globalPosition: _lastEndEdgeUpdateGlobalPosition!);
      if (currentSelectionEndIndex == -1) {
    ...
    if (_lastStartEdgeUpdateGlobalPosition != null && _hasReceivedStartEvent.add(selectable)) {
      final synthesizedEvent = SelectionEdgeUpdateEvent.forStart(globalPosition: _lastStartEdgeUpdateGlobalPosition!);
      if (currentSelectionStartIndex == -1) {
        handleSelectionEdgeUpdate(synthesizedEvent);
      }
      selectable.dispatchSelectionEvent(synthesizedEvent);
  ...
  void didChangeSelectables() {
    if (_lastEndEdgeUpdateGlobalPosition != null) {
      handleSelectionEdgeUpdate(SelectionEdgeUpdateEvent.forEnd(globalPosition: _lastEndEdgeUpdateGlobalPosition!));
    }
    if (_lastStartEdgeUpdateGlobalPosition != null) {
      handleSelectionEdgeUpdate(SelectionEdgeUpdateEvent.forStart(globalPosition: _lastStartEdgeUpdateGlobalPosition!));
    }
